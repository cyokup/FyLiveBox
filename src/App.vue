...
// 推送消息类型
const pushList = ref<string[]>(['gift'])
  ...
  messageList.value = [
    {
      id: '1',
      name: '飞鹰',
      msg: '欢迎使用飞鹰弹幕助手，输入直播地址开始安静看直播，没有刷礼物功能，所以理性看播，不要乱消费',
    },
  ]
...
// 向服务器推送消息
const PushPostMessage = async (msg: any) => {
  // 直播间地址
  const url = inputUrl.value.trim()
  msg += '&url=' + url
  ...
  await http(pushUrl.value + msg, option)
}
...
const onMessage = async (msg: any) => {
  ...
  // 解析直播消息
  await handleMessage(response.messagesList)
}

// 遍历消息数组，拿到具体的消息
const handleMessage = async (messageList: douyin.Message) => {
  messageList.forEach(async (msg) => {
    ...
    switch (msg.method) {
      ...
      case 'WebcastGiftMessage':
        console.log('礼物消息')
        await decodeGift(msg.payload)
        break
        // 聊天弹幕消息
      case 'WebcastChatMessage':
        console.log('聊天弹幕消息')
        await decodeChat(msg.payload)
        break
        // 点赞数
      case 'WebcastLikeMessage':
        console.log('点赞数')
        await likeLive(msg.payload)
        break
        // 关注消息
...
const decodeChat = async (data) => {
  ...
  // 发送http请求
  if (pushList.value.includes('chat') && pushUrl) {
    await PushPostMessage(user.nickName + '|chat-' + content)
  }
}
// 解析礼物消息
const decodeGift = async (data) => {
  ...
  // 发送http请求
  if (pushList.value.includes('gift') && pushUrl) {
    await PushPostMessage(user.nickName + '|gift-' + gift.name + '-' + repeatCount)
  }
}

// 点赞消息
const likeLive = async (data) => {
  ...
  // 发送http请求
  if (pushList.value.includes('like') && pushUrl) {
    await PushPostMessage(user.nickName + '|like-' + total)
  }
}
    ...
    <div class="setBox">
      ...
      <div class="messageSel">
        ...
        <el-checkbox-group v-model="checkList">
          <el-checkbox label="发言" value="chat"/>
      ...
      <div class="messageSel">
        <span>推送消息类型：</span>
        <el-checkbox-group v-model="pushList">
          <el-checkbox label="发言" value="chat"/>
          <el-checkbox label="礼物" value="gift"/>
          <el-checkbox label="点赞" value="like"/>
        </el-checkbox-group>
      ...
      <div class="tips">
        * 填写推送地址确认以后发送Http推送，例如：http://127.0.0.1/?msg= <br/><br/>
        * 消息格式：用户名称|消息类型-消息内容 ，例如：小明|chat-我都看傻了
      </div>
    </div>
    <template #footer>
